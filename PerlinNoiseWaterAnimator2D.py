from PIL import Image
import os
from noise import pnoise1, pnoise2, pnoise3
from math import floor, ceil, sin, sqrt, cos, atan, pi
import random

import numpy as np
import matplotlib.pyplot as plt
import matplotlib.animation as animation

cwd = os.getcwd()
shape = (100, 100)
ani_len = 50
scale = 30

speed = .02

octaves = 4
persistence = .7
lacunarity= 1.8

seed = random.randint(1, 100)

stretch_v = 1.5

repeatx, repeaty = 16, 16
repeatz = 10

def transform_coord(i, j, z, at_z):
    return i /scale - at_z, j/scale - at_z

def perlin_array(z, at_z = 0, shape = shape,
			scale = scale,octaves = octaves,
                        persistence = persistence,
                        lacunarity= lacunarity,
			seed = seed):

    if not seed:

        seed = np.random.randint(0, 100)
        print("seed was {}".format(seed))

    arr = np.zeros(shape)
    
    for i in range(shape[0]):
        for j in range(shape[1]):
            x, y = transform_coord(i, j, z, at_z)
            K = pnoise3(x,
                                        y / stretch_v,
                                        z ,
                                        octaves=octaves,
                                        persistence=persistence,
                                        lacunarity=lacunarity,
                                        repeatx=repeatx,
                                        repeaty=repeaty,
                                        repeatz = repeatz,
                                        base=seed)

            #K = 4*pow(K, 2) + 4*K + 1

            
            
            arr[i][j] = K

    basic = np.zeros(shape)
    shader = lambda i, j: arr[i][j] - arr[i][j-1]
    specular = lambda val: 2 if val > 0.995 else val
    for i in range(shape[0]):
        for j in range(shape[1]):
            
            basic[i][j] = shader(i, j)

    
            
    arr = [row[1:] for row in basic]
    max_arr = np.max(arr)
    min_arr = np.min(arr)

    norm_me = lambda x: (x-min_arr)/(max_arr - min_arr)
    
    arr = norm_me(arr)

    for i in range(shape[0]):
        for j in range(shape[1] - 1):
            arr[i][j] = specular(arr[i][j])
    
##    norm_me = lambda x: 1 - sqrt(x)
##    

    
    return arr

fig2 = plt.figure()

x = np.arange(shape[0])
y = np.arange(shape[1]).reshape(-1, 1)

frames = []

at_z = random.randint(-shape[0], 0)
for t in np.arange(0, ani_len):
    z = at_z + speed*t
    base = perlin_array(z, at_z)
    
    frames.append((plt.pcolor(x, y, base, shading='auto',cmap='viridis', vmin = -2, vmax = 5),))

im_ani = animation.ArtistAnimation(fig2, frames, interval=45, repeat = True,
                                   blit=False)

plt.show()
